...
static
void spqf_setlock(sourceinfo_t *const si,
                  const int parc,
                  char **const parv)
noexcept try
{
	if(parc < 3)
	{
		command_fail(si, fault_needmoreparams, STR_INSUFFICIENT_PARAMS, "SETLOCK");
		command_fail(si, fault_needmoreparams, _("Syntax: SETLOCK <vote type> <category> <parameter> <value>"));
		return;
	}
	...
	Metadata md(metadata,Cfg::OPERLOCK_PREFIX);
...
command_t cmd_setlock
{
	"SETLOCK",
	N_(N_("Lock a specific parameter to your value for all channels.")),
	PRIV_USER_AUSPEX,
	4,
	spqf_setlock,
	{ nullptr, nullptr }
...
noexcept
{
	...
	service_bind_command(spqf->svc,&cmd_setlock);
}
...
noexcept
{
	service_unbind_command(spqf->svc,&cmd_setlock);
